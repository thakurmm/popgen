...
- run the script:
	...
	python create_admixed_chromosomes.py --num_admixed_chromosomes 200 --source_pops CEU YRI
...
def create_test_chromosome(chr_strands, pops, ref_IDs):
	...
	stop = pd.Series(range(NUM_RECOMBINATIONS)).map(lambda x: np.where(chr_strands['POS'] == recombination_spots[x])[0][0]+1)
	start = pd.concat([pd.Series(0), stop], ignore_index=True)
	stop = pd.concat([stop, pd.Series(len(chr_strands))], ignore_index=True)
...
if __name__ == '__main__':
	...
	proportions_out_filename = '/home/greg/School/popgen/data/admixed/{}_{}_admixed_{}admixed_{}pure_proportions.txt'.format(sourcepop1, sourcepop2, num_admixed_chromosomes, num_anchor)

	sourcepop1_ids = get_pop_IDs(sourcepop1_ids_filename)
	sourcepop2_ids = get_pop_IDs(sourcepop2_ids_filename)

	all_chr_strands = pd.read_csv(homologous_filename, sep='\t', header=0, comment='#')
